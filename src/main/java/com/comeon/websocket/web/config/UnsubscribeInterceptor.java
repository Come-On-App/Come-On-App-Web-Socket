...
public class UnsubscribeInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.UNSUBSCRIBE.equals(headerAccessor.getCommand())) {
            log.debug("subId {} unsubscribe.....", headerAccessor.getSubscriptionId());
            Map<String, Object> sessionAttributes = headerAccessor.getSessionAttributes();
            Map<String, String> subscriptions = (Map<String, String>) sessionAttributes.get("subscriptions");
            String destination = subscriptions.get(headerAccessor.getSubscriptionId());
            ...
            if (pathMatcher.match(MEETING_SUBSCRIBE_PATH_PATTERN, destination)) {
                ...
                log.debug("userId: {}, meetingId: {}", userId, meetingId);

                String sessionId = headerAccessor.getSessionId();
                String subscriptionId = headerAccessor.getSubscriptionId();
                log.debug("sessionId: {}, subscriptionId: {}", sessionId, subscriptionId);
                boolean result = meetingSubscriptionRepository.removeSession(meetingId, userId, sessionId, subscriptionId);
                if (result) {
